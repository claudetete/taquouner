...
(when tqnr-section-display
  ...
  (when tqnr-section-display-font
    ...
    ;;
    (setq tqnr-profile-font "DejaVu Sans Mono-8")
...
(when tqnr-section-interface
  ...
  (when tqnr-section-interface-main-window
    ...
    (setq tqnr-profile-window-title "<GNU Emacs>")
    ;;
...
(when tqnr-section-shortcut
  ...
  (setq tqnr-section-shortcut-etag nil)

  ;; SEMANTIC: add shortcut to move in source code with semantic
  (setq tqnr-section-shortcut-semantic nil)
...
(when tqnr-section-misc
  ...
  (setq tqnr-section-misc-bookmark nil)
...
(provide 'profile-simple)

;;; profile-simple.el ends here
