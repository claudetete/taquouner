...
;; Copyright (c) 2013-2018 Claude Tete
;;
...
(try-require 'profile-sagem-defense-private "    ")
...
(when tqnr-section-mode
  ...
  (setq tqnr-section-mode-doxymacs t)
  ...
  (when tqnr-section-mode-org-mode
    ...
    (setq tqnr-profile-org-directory "D:/CTE/org")
    ;; default org file where all task/todo capture will goes
    (setq tqnr-profile-org-default-notes-file (concat (file-name-as-directory tqnr-profile-org-directory) "KC390.org"))
  ...
  (setq tqnr-section-mode-synergy t)
  (when tqnr-section-mode-synergy
    ;; login to connect to synergy server
    (setq tqnr-profile-synergy-username "FT346598")
    ;; database path to connect to synergy server
    (setq tqnr-profile-synergy-database "/opt/ccm/databases/kc390_spds") ;; KC390
    ;;(setq tqnr-profile-synergy-database "/opt/ccm/databases/trcu") ;; TRCU
    ;; server url to connect to synergy server
    (setq tqnr-profile-synergy-server "http://synergy7c.sds.safran:8410") ;; KC390
    ;;(setq tqnr-profile-synergy-server "http://synergy7a.sds.safran:8410") ;; TRCU
...
(when tqnr-section-annoyances
  ...
  (when tqnr-section-annoyances-backup-file-in-directory
    ...
    (setq tqnr-profile-backup-directory "D:/tmp/emacs")
    (setq tqnr-profile-autosave-directory "D:/tmp/emacs")
    ) ;; (when tqnr-section-annoyances-backup-file-in-directory
  ) ;; (when tqnr-section-annoyances
...
(when tqnr-section-misc
  ...
  (setq tqnr-profile-browser "D:/CTE/tools/LiberKey/MyApps/Vivaldi/Application/vivaldi.exe")
  ;;
...
(provide 'profile-sagem-defense)

;;; profile-sagem-defense.el ends here
