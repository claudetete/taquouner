...
(when section-environment
  ...
  (when section-environment-set-path
    ...
    (setq profile-path
      (concat
        program-dir "/MinGW/msys/1.0/bin"   ";"
        program-dir "/cywin/bin"            ";"
        program-dir "/cygwin/usr/bin"       ";"
        program-dir "/cygwin/usr/local/bin" ";"
        program-dir "/gnuwin32/bin"         ";"
        program-dir "/MikTex/miktex/bin"    ";"
        "c:/WINDOWS"                        ";"
        "c:/WINDOWS/System32"               ";"
        )
      )
    ...
    (setq profile-exec-path
      '(
         (concat program-dir "/MinGW/msys/1.0/bin"  )
         (concat program-dir "/cywin/bin"           )
         (concat program-dir "/cygwin/usr/bin"      )
         (concat program-dir "/cygwin/usr/local/bin")
         (concat program-dir "/MikTex/miktex/bin"   )
         "c:/WINDOWS"
         "c:/WINDOWS/System32"
         )
      )
  ...
  (when section-environment-executable
    ;; shell
    (setq profile-shell-file-name (concat program-dir "/cygwin/bin/bash.exe"))
    ;; ediff
    (setq profile-ediff-diff-program (concat program-dir "/GnuWin32/bin/diff.exe"))
    (setq profile-ediff-diff3-program (concat program-dir "/GnuWin32/bin/diff3.exe"))
    (setq profile-ediff-cmp-program (concat program-dir "/GnuWin32/bin/cmp.exe"))
    ) ; (when section-environment-executable
