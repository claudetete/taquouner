...
(when section-environment
  ...
  (when section-environment-set-path
    ...
    (setq profile-path
      (concat
        (concat (file-name-as-directory dotemacs-path) "plugins/pt_linux_amd64") ";"
        )
      )
    ;; executables path
  ...
  ;; REQUIREMENT: `section-environment-os-recognition'
  (setq section-environment-ms-windows-performance nil)
  ;; EXECUTABLE: set some executable like shell or diff
  ;; REQUIREMENT: `section-environment-os-recognition'
  ;;              `section-environment-terminal-vs-graphics'
  (setq section-environment-executable nil)
...
(when section-mode
  ...
  (when section-mode-vc-clearcase
    ...
    (setq profile-clearcase-vtree "clearvtree")
    ;; path to cleartool executable
    (setq profile-cleartool "cleartool")
    ) ; (when section-mode-vc-clearcase
...
(when section-misc
  ...
  ;; backup directory
  (setq profile-backup-directory "~/.emacs.d/trash")
  (setq profile-autosave-directory "~/.emacs.d/trash")
...
(provide 'profile-archlinux)

;;; profile-archlinux.el ends here
