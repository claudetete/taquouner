...
(when tqnr-section-environment
  ...
  (when tqnr-section-environment-set-path
    ...
    (setq tqnr-profile-exec-path
      '(
         (concat (file-name-as-directory tqnr-dotemacs-path) "plugins/pt_linux_amd64")
         )
      )
  ...
  (setq tqnr-section-environment-executable nil)
...
(when tqnr-section-annoyances
  ...
  (when tqnr-section-annoyances-backup-file-in-directory
    ...
    (setq tqnr-profile-backup-directory "~/.emacs.d/trash")
    (setq tqnr-profile-autosave-directory "~/.emacs.d/trash")
    ) ;; (when tqnr-section-annoyances-backup-file-in-directory
  ) ;; (when tqnr-section-annoyances
