...
(defun gtags-completing (flag string predicate code)
  ...
  (let ((option (cond ((eq flag 'files) "-Pon")
    ...
    (call-process "d:/cygwin/usr/local/bin/global.exe" nil t nil option string)
    (goto-char (point-min))
    ;
...
(defun gtags-get-rootpath ()
  (let (path buffer)
    (save-excursion
      ...
      (set-buffer buffer)
      (setq n (call-process "d:/cygwin/usr/local/bin/global.exe" nil t nil "-pr"))
      (if (= n 0)
...
(defun gtags-display-browser ()
  ...
  (interactive)
  (call-process "d:/cygwin/usr/local/bin/gozilla.exe"  nil nil nil (concat "+" (number-to-string (gtags-current-lineno))) buffer-file-name))
...
(defun gtags-goto-tag (tagname flag &optional other-win)
  (let (option context save prefix buffer lines flag-char)
    ...
    (if (not (= 0 (if (equal flag "C")
                      (call-process "d:/cygwin/usr/local/bin/global.exe" nil t nil option "--encode-path=\" \t\"" context tagname)
;                      (message "%s %s %s" option context tagname)
                      (call-process "d:/cygwin/usr/local/bin/global.exe" nil t nil option "--encode-path=\" \t\"" tagname)
;                      (message "%s %s" option tagname)
                    )))
